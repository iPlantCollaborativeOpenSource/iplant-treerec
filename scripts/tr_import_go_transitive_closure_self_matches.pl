e#!/usr/bin/perl -w
#
# THIS PROGRAM SHOULD BE DEPRECATED, AND THE RELEVANT CODE
# MOVED TO tr_import_go_transitive_closure
# This is only here because I did not include the self
# matches in the initial import of these data.
#
#-----------------------------------------------------------+
#                                                           |
...
# ADDED REFLEXIVE TRANSITIVE CLOSURES
# THIS IS THE X is_a X PARTS OF THE RELATIONSHIPS
# ie nucleious is_a nucleus
# This may sound silly but will make the queries a bit more straightforward
#
#02/14/2011

# GET THE IS_Relation ID
my $db_is_rel_id = gorel_2_cvterm_id ($dbh,"is_a");

#-----------------------------+
# GET THE FULL LIST OF GO TERMS IDS
#-----------------------------+

my $get_go_ids_sql = "SELECT cvterm_id".
    " FROM cvterm".
    " LEFT JOIN dbxref ON".
...
my $get_go_ids_sth = $dbh->prepare($get_go_ids_sql);
$get_go_ids_sth->execute();

while (my $r = $get_go_ids_sth->fetchrow_arrayref) {
    my $cvterm_id = @$r[0];
    print STDERR "\t$cvterm_id --- $db_is_rel_id\n";    
    # INSERT SELF TERMS INTO cvtermpath
...
# EXIT OUT FOR NOW, JUST DOING THE IS SELF MATCHES
exit;
...
while (<INFILE>) {
    ...
    # is_a sinc we are not selecting by database
...
This documentation refers to program version $Rev: 627 $
